...
const MODULE_DOCS = {
  ...
  channels: {
    title: "Channel Mix", docRef: "/docs/modules/channels",
    tooltip: "Spend allocation across demand gen channels",
    tldr: "Distributes the programmatic budget across channels (paid, content syndication, events, etc.) based on allocation weights. Shows CPL and volume per channel.",
    included: ["Channel spend allocation", "CPL per channel", "Inquiry volume per channel", "Blended CPL"],
    excluded: ["Martech costs (see Mktg Budget)", "Fixed marketing overhead"],
    assumptions: ["CPL benchmarks based on mid-market cyber", "Channel mix allocation is manual (not optimized)", "All channels feed the same funnel"],
    whatChanges: ["Variable marketing budget (% of revenue)", "Channel allocation weights", "CPL per channel"],
    relatedModules: ["mktgBudget", "pipeline", "cacBreakdown"],
  },
  mktgBudget: {
    title: "Marketing Budget", docRef: "/docs/modules/marketing-budget",
    tooltip: "Budget decomposition with stress testing and debt tax",
    tldr: "Shows where marketing dollars actually go: programmatic (buys pipeline), martech (enables targeting), and fixed overhead (leadership, ops, staff, tools). Stress modes model lean vs transformation scenarios.",
    included: ["Variable vs fixed split", "Programmatic vs martech decomposition", "Stress modes (lean/normal/transform)", "Tactical debt tax", "CAC layers", "Sensitivity analysis"],
    excluded: ["Sales budget (see S&M Budget)", "Channel-level detail (see Channel Mix)"],
    assumptions: ["Fixed marketing has a headcount floor (VP comp is a step function)", "Martech is % of variable budget", "Debt tax applies as surcharge on fixed overhead", "Stress modes change fixed overhead % only"],
    whatChanges: ["Variable mktg % of revenue", "Fixed mktg % of total", "Martech % of variable", "Stress mode selection", "Debt tax slider", "Funding stage (changes VP comp)"],
